...
const peerSocketConnection = (server) => {
  ...
  io.on("connection", socket => {
    socket.on("join-room", (roomID, userID, displayName, roomName, a) => {
      console.log(roomID, userID, displayName, roomName, a)
      socket.join(roomID);
      ...
      socket.on("position", (id, position) => {
        io.to(roomID).emit("position", id, position);
      });
